...
const Mapping = () => {
  ...
  const getLocations = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/api/location-by-bloodGroup?bloodGroup=${encodeURIComponent(selectedBloodGroup)}`
      );
  ...
  return (
    <>
      ...
      {loading && <p className="text-center text-red text-lg font-semibold p-3">Fetching locations...</p>}

      <div className="bg-whitee dark:bg-black p-10 d-flex justify-center items-center" id="map" ref={mapSectionRef}>

        <MapContainer center={[userLocation.lat || 51.505, userLocation.lon || -0.09]} zoom={8} style={{ height: "400px", width: "100%" }}>
          ...
          {pinnedLocation.map((loc, key) => (
            <Marker key={key} position={[loc.location[0].lat, loc.location[0].lon]} icon={redMarker}>
              <Popup>
                <ul className="flex flex-col gap-2">
                  {loc.bloodInventory.map((blood, idx) => (
                    <li key={idx}>
                      Type: {blood.bloodType} | Quantity: {blood.quantity} bottles
                    </li>
                  ))}
                  <li>Blood Bank: {loc.name}</li>
                  <li>
                    {isLoggedIn ? <BookBloodModal bloodInventory={loc.bloodInventory} organizationID={loc._id} /> : navigate("/loginReg")}
                  </li>
                </ul>
              </Popup>
            </Marker>
