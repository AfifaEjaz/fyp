...
export async function registeration(req, res) {
    ...
    const { name, email, password, password_confirmation, role, tc } = req.body
...
export async function getAllOrganization(req, res) {
    console.log("chala");
    try {
        const result = await user.find({ role: "organization" })
